...
export default function Cart() {
  ...
  return (
    <div className="p-8 max-w-4xl mx-auto w-full">
      <h2 className="text-3xl font-bold mb-6 text-center text-gray-300">🛒 {t('yourCart')}</h2>
      {cart.length === 0 ? (
            ...
            {cart.map((drink) => {
              ...
              return (
                <li key={drink.id} className="relative flex flex-col md:flex-row items-stretch gap-4 bg-white/60 p-4 rounded-xl shadow-lg">
                  ...
                  <button onClick={() => removeFromCart(drink.id)} className="absolute top-2 right-3 text-red-500 hover:text-red-700 font-bold text-2xl" title="Удалить из корзины">✕</button>
                </li>
              );
            })}
          </ul>
